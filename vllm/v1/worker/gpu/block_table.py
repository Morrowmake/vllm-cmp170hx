...
class BlockTables:
    def __init__(
        ...
        # DCP-sharded (full-attention / MLA, including the pooled DSA indexer)
        # get localized slot mappings. Groups with replicated per-rank state --
        # the kpool tail scratch (KpoolTailSpec) and Mamba/KDA -- keep their
        # global positions; localizing them would silently corrupt their
        # caches. Defaults to all-enabled, which reproduces the previous
        # behaviour exactly.
...
def _compute_slot_mappings_kernel(
    ...
    for i in range(start_idx, end_idx, TRITON_BLOCK_SIZE):
        ...
        if CP_SIZE == 1:
            ...
            # Context parallelism is used, but only for the groups whose KV is
            # actually sharded across DCP ranks. Groups with replicated state
            # (kpool tail scratch, Mamba/KDA) keep their global positions and
            # own every token.
